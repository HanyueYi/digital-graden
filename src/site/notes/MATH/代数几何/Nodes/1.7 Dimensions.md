...
Firstly, note that $\dim U_0=\mathrm{tr}.\deg_k \mathrm{Frac}(R)$. On the other hand, recall (我反正没学过) that for a closed set $V(f)$ which has irreducible components $V(f)=Z_1'\cup\cdots\cup Z_\ell'$, then each $Z_k'$ corresponds a minimal prime ideals $p_k=I(Z_k')$ containing $(f)$. So $Z_0=V(p)$ where $p$ is the minimal prime ideal containing $f$, and we have $\Gamma(Z_0)\simeq R/p$ and $k(Z_0)=\mathrm{Frac}(R/p)$. Then by [[MATH/代数几何/Nodes/1.7 Dimensions#^a5bed9\|#^a5bed9]] there is 
