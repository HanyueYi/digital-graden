---
{"dg-publish":true,"draft":false,"permalink":"/MATH/抽象代数III/Nodes/Module Constructions and Universal Properties/","dgPassFrontmatter":true}
---
...
**_Proof._**
i) Note that $R\to\mathrm{End}(M)$ factorizes through the natural surjection $R\to R/I$ means there exists $\rho_I:R/I\to \mathrm{End}(M)$ such that the diagram 

![Pasted image 20250503145513.png|200](/img/user/%E9%99%84%E4%BB%B6/Pasted%20image%2020250503145513.png)

commutes. By the universal property of ring homomorphism, $\rho_I$ exists iff $I\subseteq \ker \rho$ iff $IM=0$. 

ii) We define $R/I$ acting on $M/IM$ as $(r+I)\cdot(m+IM)=rm+IM$, then it is trivial to check $M/IM$ is a $R/I$-module. 

iii) For any $x\in IM$, $x$ can be written as a finite sum of $x=\sum_ja_jm_j$. Then 

$$\varphi(x)=\sum\varphi(a_jm_j)=\sum a_j\varphi(m_j)\in IM'$$

and so $\varphi(IM)\subseteq I M'$. Define $\overline \varphi:M/IM\to M'/IM'$ naturally, and it is easy to check it is a morphism between $R/I$-modules. 
<p align="left">□</p>
...
**Remark.**  Modules $M$ and $N$ are defined as above. The pair $(M/N,\pi_N)$ is **unique** in the following sense.
- If $(M',\pi')$ is a pair such that 
  ...
  commutes,
- then $M'\simeq M/N$. 
- The proof is as follows: take $L'=M/N$, then there exists a unique isomorphism $\sigma:M/N\stackrel{\sim}{\to} M'$ such that the following diagram commutes, where $\sigma=\varphi_{M'}'^{-1}$. 
...
> [!definition]
> Let $I$ be a set. Write $R^I=\prod_{i\in I}R$ and $R^{(I)}=\oplus_{i\in I}R$. Let $M$ be a $R$-module. Take $x_i$, $i\in I$ be a family of elements of $M$. The submodule generated by $(x_i)_{i\in I}$ is denoted by $\sum_{i\in I}Rx_i$, and we have a morphism of $R$-modules 
> 
> $$\varphi:R^{(I)}\to M,(r_i)_{i\in I}\mapsto \sum_{i\in I}r_ix_i.$$
> 
> Then
> - $\varphi$ injective iff $x_i,i\in I$ are linearly independent;
> - $\varphi$ surjective iff $x_i$, $i\in I$ generate $M$;
> - $\varphi$ isomorphism iff $M$ is free with a basis $\{x_i\}_{i\in I}$.
...
**Remark.** Up to isomorphism, tensor product exists and it is unique. See [[MATH/交换代数/Nodes/2 Modules#^aa5afc\|2 Modules#^aa5afc]]. 
...
**Remark.** Recall [[MATH/交换代数/Nodes/2 Modules#Strange Things about Tensor Product\|2 Modules#Strange Things about Tensor Product]]. 

> [!proposition] boring thing
...
**_Proof._**
See [[MATH/交换代数/Nodes/2 Modules#^60fa98\|2 Modules#^60fa98]].
<p align="left">□</p>

**Remark.** The meaning of "canonical": for any morphism $f:M'\to M$, the following diagram commutes. 

![Pasted image 20250503162021.png|600](/img/user/%E9%99%84%E4%BB%B6/Pasted%20image%2020250503162021.png)
...
**Example.** Let $V$ be an $n$-dimensional $\mathbb{Q}$-space, i.e., $V\simeq \mathbb{Q}^n$, then $\mathbb{C}\otimes_\mathbb{Q}V\simeq_\mathbb{Q} \mathbb{C}\otimes (\mathbb{Q}\oplus\cdots\oplus \mathbb{Q})\simeq(\mathbb{C}\otimes_\mathbb{Q}\mathbb{Q})^n\simeq \mathbb{C}^n$.
